...
import com.delightlane.keyboard.hangul.HangulEngine.HangulEngineEvent;
import com.delightlane.keyboard.hangul.HangulEngine.HangulEngineListener;
import com.delightlane.keyboard.hangul.HangulEngine.SetComposingEvent;
import com.delightlane.keyboard.preference.KeystrokePreference;
import com.delightlane.keyboard.view.ListLangKeyActionDialogActivity;
import com.delightlane.keyboard.hangul.TwelveHangulEngine;

import com.delightlane.keyboard.event.CommitComposingTextEvent;
import com.delightlane.keyboard.event.EngineModeChangeEvent;
import com.delightlane.keyboard.event.InputCharEvent;
import com.delightlane.keyboard.event.InputKeyEvent;
import com.delightlane.keyboard.event.InputSoftKeyEvent;
import com.delightlane.keyboard.event.InputTimeoutEvent;
import com.delightlane.keyboard.event.InputViewChangeEvent;
import com.delightlane.keyboard.event.KeyUpEvent;
import com.delightlane.keyboard.event.SoftKeyFlickEvent;
import com.delightlane.keyboard.event.SoftKeyGestureEvent;
...
public class SebeolHangulIME extends InputMethodService implements HangulEngineListener {
	...
	public View onCreateInputView() {
        ...
        boolean hardKeyboardHidden = ((com.delightlane.keyboard.DefaultSoftKeyboard) mInputViewManager).mHardKeyboardHidden;
...
//			if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) updateNavigationBar();
	...
	public void onSoftKeyLongPress(SoftKeyLongPressEvent event) {
		...
		if(keyCode < 0) {
			...
			case com.delightlane.keyboard.DefaultSoftKeyboard.KEYCODE_CHANGE_LANG:
	...
	private void flickAction(String flickAction, int keyCode) {
		...
		case FLICK_SHIFT: {
			switch(keyCode) {
			case com.delightlane.keyboard.DefaultSoftKeyboard.KEYCODE_QWERTY_SHIFT:
				break;
	...
	public void updateMetaKeyStateDisplay() {
		...
		if (mHardShift == 0 && mHardAlt == 0){
			mode = com.delightlane.keyboard.DefaultSoftKeyboard.HARD_KEYMODE_SHIFT_OFF_ALT_OFF;
		} else if(mHardShift == 1 && mHardAlt == 0) {
			mode = com.delightlane.keyboard.DefaultSoftKeyboard.HARD_KEYMODE_SHIFT_ON_ALT_OFF;
		} else if(mHardShift == 2  && mHardAlt == 0) {
			mode = com.delightlane.keyboard.DefaultSoftKeyboard.HARD_KEYMODE_SHIFT_LOCK_ALT_OFF;
		} else if(mHardShift == 0 && mHardAlt == 1) {
			mode = com.delightlane.keyboard.DefaultSoftKeyboard.HARD_KEYMODE_SHIFT_OFF_ALT_ON;
		} else if(mHardShift == 0 && mHardAlt == 2) {
			mode = com.delightlane.keyboard.DefaultSoftKeyboard.HARD_KEYMODE_SHIFT_OFF_ALT_LOCK;
		} else if(mHardShift == 1 && mHardAlt == 1) {
			mode = com.delightlane.keyboard.DefaultSoftKeyboard.HARD_KEYMODE_SHIFT_ON_ALT_ON;
		} else if(mHardShift == 1 && mHardAlt == 2) {
			mode = com.delightlane.keyboard.DefaultSoftKeyboard.HARD_KEYMODE_SHIFT_ON_ALT_LOCK;
		} else if(mHardShift == 2 && mHardAlt == 1) {
			mode = com.delightlane.keyboard.DefaultSoftKeyboard.HARD_KEYMODE_SHIFT_LOCK_ALT_ON;
		} else if(mHardShift == 2 && mHardAlt == 2) {
			mode = com.delightlane.keyboard.DefaultSoftKeyboard.HARD_KEYMODE_SHIFT_LOCK_ALT_LOCK;
		} else {
			mode = com.delightlane.keyboard.DefaultSoftKeyboard.HARD_KEYMODE_SHIFT_OFF_ALT_OFF;
		}
		((com.delightlane.keyboard.DefaultSoftKeyboard) mInputViewManager).updateIndicator(mode);
		mode = DefaultSoftKeyboard.HARD_KEYMODE_LANG
				+ ((com.delightlane.keyboard.DefaultSoftKeyboard) mInputViewManager).mCurrentLanguage;
		((com.delightlane.keyboard.DefaultSoftKeyboard) mInputViewManager).updateIndicator(mode);
	}
